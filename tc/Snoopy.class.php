...
class Snoopy
{
	...
	function _prepare_post_body($formvars, $formfiles)
	{
		...
		switch ($this->_submit_type) {
			case "application/x-www-form-urlencoded":
				reset($formvars);
				$postdata .= http_build_query($formvars, null, '&');
			...
			case "multipart/form-data":
				...
				$postdata = $this->getPostBody($formvars);
				...
				$postdata .= $this->getFilesPostBody($formfiles);
	...
	function getFilesPostBody($aVars, $sNamePrefix=false) {

		$sPostdata = '';

		foreach((array)$aVars as $sKey=>$mVar) {

			if($sNamePrefix === false) {
				$sCurrentPrefix = $sKey;
			} else {
				$sCurrentPrefix = $sNamePrefix.'['.$sKey.']';
			}

			if (is_array($mVar) || is_object($mVar)) {
				$sPostdata .= $this->getFilesPostBody($mVar, $sCurrentPrefix);
			} else {
				if(!is_readable($mVar))
				{
					continue;
				...
				$fp = fopen($mVar, "r");
				$file_content = fread($fp, filesize($mVar));
				fclose($fp);
				$base_name = basename($mVar);

				$sPostdata .= "--".$this->_mime_boundary."\r\n";
				$sPostdata .= "Content-Disposition: form-data; name=\"$sCurrentPrefix\"; filename=\"$base_name\"\r\n\r\n";
				$sPostdata .= "$file_content\r\n";
			}
		}
	...
	function getPostBody($aVars, $sNamePrefix=false) {
		...
		foreach((array)$aVars as $sKey=>$mVar) {
			...
			if (is_array($mVar) || is_object($mVar)) {
				$sPostdata .= $this->getPostBody($mVar, $sCurrentPrefix);
			} else {
				$sPostdata .= "--".$this->_mime_boundary."\r\n";
				$sPostdata .= "Content-Disposition: form-data; name=\"".$sCurrentPrefix."\"\r\n\r\n";
				$sPostdata .= "$mVar\r\n";
			}
		}
